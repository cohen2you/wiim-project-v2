...
"Investors are looking ahead to the company's next earnings report, scheduled for ${typeof nextEarnings === 'object' && nextEarnings.date ? new Date(nextEarnings.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : nextEarningsDate ? new Date(nextEarningsDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'a date to be announced'}, with analysts expecting earnings per share of $${typeof nextEarnings === 'object' && nextEarnings.eps_estimate ? parseFloat(nextEarnings.eps_estimate.toString()).toFixed(2) : 'N/A'}${typeof nextEarnings === 'object' && nextEarnings.eps_prior ? `, ${parseFloat(nextEarnings.eps_estimate?.toString() || '0') > parseFloat(nextEarnings.eps_prior.toString()) ? 'up from' : parseFloat(nextEarnings.eps_estimate?.toString() || '0') < parseFloat(nextEarnings.eps_prior.toString()) ? 'down from' : 'compared to'} $${parseFloat(nextEarnings.eps_prior.toString()).toFixed(2)} in the previous quarter` : ''}${typeof nextEarnings === 'object' && nextEarnings.revenue_estimate && nextEarnings.revenue_prior ? ` and revenue of $${(parseFloat(nextEarnings.revenue_estimate.toString()) / 1000000).toFixed(2)}M${parseFloat(nextEarnings.revenue_estimate.toString()) > parseFloat(nextEarnings.revenue_prior.toString()) ? ', up from' : parseFloat(nextEarnings.revenue_estimate.toString()) < parseFloat(nextEarnings.revenue_prior.toString()) ? ', down from' : ', compared to'} $${(parseFloat(nextEarnings.revenue_prior.toString()) / 1000000).toFixed(2)}M in the prior period` : ''}. At current levels, the P/E ratio of ${peRatio.toFixed(1)} ${peRatio > 25 ? 'suggests the stock may be overvalued relative to peers' : peRatio < 15 ? 'suggests the stock may offer value relative to peers' : 'suggests the stock is fairly valued relative to peers'}, which investors will be watching closely as earnings approach."
` : nextEarnings ? `
"Investors are looking ahead to the company's next earnings report, scheduled for ${typeof nextEarnings === 'object' && nextEarnings.date ? new Date(nextEarnings.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : nextEarningsDate ? new Date(nextEarningsDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'a date to be announced'}, ${typeof nextEarnings === 'object' && nextEarnings.eps_estimate ? `with analysts expecting earnings per share of $${parseFloat(nextEarnings.eps_estimate.toString()).toFixed(2)}${typeof nextEarnings === 'object' && nextEarnings.eps_prior ? `, ${parseFloat(nextEarnings.eps_estimate.toString()) > parseFloat(nextEarnings.eps_prior.toString()) ? 'up from' : parseFloat(nextEarnings.eps_estimate.toString()) < parseFloat(nextEarnings.eps_prior.toString()) ? 'down from' : 'compared to'} $${parseFloat(nextEarnings.eps_prior.toString()).toFixed(2)} in the previous quarter` : ''}${typeof nextEarnings === 'object' && nextEarnings.revenue_estimate && nextEarnings.revenue_prior ? ` and revenue of $${(parseFloat(nextEarnings.revenue_estimate.toString()) / 1000000).toFixed(2)}M${parseFloat(nextEarnings.revenue_estimate.toString()) > parseFloat(nextEarnings.revenue_prior.toString()) ? ', up from' : parseFloat(nextEarnings.revenue_estimate.toString()) < parseFloat(nextEarnings.revenue_prior.toString()) ? ', down from' : ', compared to'} $${(parseFloat(nextEarnings.revenue_prior.toString()) / 1000000).toFixed(2)}M in the prior period` : ''}.` : 'which will provide key insights into the company\'s financial performance and outlook.'}"
` : consensusRatings ? `
...
- DO NOT include a raw timestamp (e.g., "10:30 AM" or "5:59 PM ET") - the article header already displays this
- DO specify the Trading Session clearly: ${marketStatus === 'premarket' ? `Use phrases like "in ${dayOfWeek}'s pre-market trading" or "during pre-market trading on ${dayOfWeek}"` : marketStatus === 'afterhours' ? `Use phrases like "in after-hours trading on ${dayOfWeek}" or "during ${dayOfWeek}'s after-hours session"` : marketStatus === 'open' ? `Use phrases like "during ${dayOfWeek}'s session" or "in ${dayOfWeek}'s trading session"` : `Use phrases like "on ${dayOfWeek}" or reference the previous trading session appropriately`}
