...
Return the complete enhanced story with integrated X post context:`;
    ...
    return NextResponse.json({ story: enhancedStory, postsUsed: selectedPosts.length });
  } catch (error: any) {
    console.error('Error adding X posts to story:', error);
    return NextResponse.json({ error: error.message || 'Failed to add X posts to story' }, { status: 500 });
  }
}
