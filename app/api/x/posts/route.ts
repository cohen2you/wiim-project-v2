...
export async function POST(req: Request) {
  try {
    const { topic, count = 10, minFollowers = 1000 } = await req.json(); // Added minFollowers
    ...
    const searchUrl = `${X_API_URL}?query=${encodeURIComponent(searchQuery)}&max_results=${Math.min(count * 3, 100)}&tweet.fields=created_at,author_id,public_metrics,context_annotations&user.fields=username,name,verified,public_metrics&expansions=author_id&media.fields=url,preview_image_url`; // Increased max_results to filter, added public_metrics for users
    ...
    return NextResponse.json({ posts: tweets, total_found: tweets.length, topic: topic });
  } catch (error: any) {
    console.error('Error fetching X posts:', error);
    return NextResponse.json({ error: error.message || 'Failed to fetch X posts' }, { status: 500 });
  }
}
